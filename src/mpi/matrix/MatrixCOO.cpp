...
long long int tbsla::mpi::MatrixCOO::read_bin_mpiio(MPI_Comm comm, std::string filename, long long int pr, long long int pc, long long int NR, long long int NC) {
  int world, rank;
  ...
  */
  ...
  long long int s = tbsla::utils::range::pflv(vec_size, pr * NC + pc, NR * NC);
...
double* tbsla::mpi::MatrixCOO::spmv(MPI_Comm comm, const double* v, long long int vect_incr) {
 ...
 *
 */
inline void tbsla::mpi::MatrixCOO::Ax(MPI_Comm comm, double* r, const double* v, double *buffer, double *buffer2, long long int vect_incr) {
  this->Ax(buffer, v, vect_incr);
  MPI_Allreduce(buffer, r, this->n_row, MPI_DOUBLE, MPI_SUM, comm);
}

double* tbsla::mpi::MatrixCOO::a_axpx_(MPI_Comm comm, const double* v, long long int vect_incr) {
  double* r = this->spmv(comm, v, vect_incr);
  std::transform (r, r + this->n_row, v, r, std::plus<double>());
  return this->spmv(comm, r, vect_incr);
}

double* tbsla::mpi::MatrixCOO::page_rank(MPI_Comm comm, double beta, double epsilon, long long int max_iterations, long long int &nb_iterations_done){
  int proc_rank;
