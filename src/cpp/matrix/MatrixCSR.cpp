...
void tbsla::cpp::MatrixCSR::fill_cdistrib(long long int n_row, long long int n_col, long long int  nnz, long long int pr, long long int pc, long long int NR, long long int NC) {
  ...
  long long int size = 2 * ceil((double)nnz * (double)ln_row / (double)(this->NC));
  this->values = new double[size];
  this->colidx = new long long int[size]();
  this->rowptr = new long long int[ln_row + 1]();
  ...
  for(long long int i = 0; i < ln_row; i++) {
    ...
    for (long long int j = 0; j < nnz ; j ++){
      col = (f_row + i + (long long int)round((double)j * (double)this->n_row / (double)nnz)) % this->n_row;
      if( col >= f_col && col < f_col + ln_col ){
	if(this->rowptr[i + 1] >= size){
          double * tempsvalues=new double[size + 3];
          long long int * tempscolidx=new long long int [size + 3];
